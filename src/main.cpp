...
#if defined(NDEBUG)
# error "Currycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x176b767825a8b2d80d1e1caeda30211cc7d9ac03e58377994398c6b0a806cdbd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Currycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Currycoin Signed Message:\n";
...
{
    // Currycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1500000 blocks, which will occur approximately every 40000 years
    nSubsidy >>= (nHeight / 1500000); // Currycoin: 1500000 blocks in ~40000 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Currycoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Currycoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Currycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Currycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xdd5c284e3d147a30c5bfca078e3f42d5177752e230c820ace83a8ece3a09942f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Mudslide devastation in California";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04afe7621fc21c46e814431e2b7f424e862947db1116461774c225e57cd159dd3941cfd105efea8e082f517110d42f77189b33af382cdd92ed574dd1c1f779cb91") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515581114;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086926491;

        if (fTestNet)
        {
            block.nTime    = 1515581062;
            block.nNonce   = 387129891;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa8ec32b3546be34476be7a0745abebecb4ece345367d771b9cffb8af6db540f4"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc3, 0xb1, 0xda }; // Currycoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CurrycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CurrycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CurrycoinMiner(CWallet *pwallet)
{
    printf("CurrycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("currycoin-miner");
    ...
    try { loop {
        ...
        printf("Running CurrycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CurrycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CurrycoinMiner, pwallet));
}
